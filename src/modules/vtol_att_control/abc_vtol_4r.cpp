...
/**
* @file abc_vtol_4r.cpp
* @author wangsl 	<wangsenlin.cn@gmail.com>
* @brief 四旋翼无人机（AbcVtol4R4R）VTOL控制实现
*
* 四旋翼无人机是一种垂直起降飞行器，通过改变姿态实现多旋翼和固定翼模式间的转换。
*/
...
// 构造函数
AbcVtol4R::AbcVtol4R(VtolAttitudeControl *attc)
	: VtolType(attc) // 调用基类构造函数
{}

// 参数更新函数
void AbcVtol4R::parameters_update() {
	VtolType::updateParams(); // 调用基类参数更新
}

// 更新VTOL状态函数
void AbcVtol4R::update_vtol_state() {
	/* 使用双向开关进行模式转换的简单逻辑
...
// 更新转换状态函数
void AbcVtol4R::update_transition_state()
{
	...
	// 回退转换开始时混合推力
	if (_vtol_mode == vtol_mode::TRANSITION_BACK) {
		const float progress = math::constrain(_time_since_trans_start / ABC4R_B_TRANS_THRUST_BLENDING_DURATION,
			0.f, 1.f);
...
// 等待TECS（总能量控制系统）函数
void AbcVtol4R::waiting_on_tecs() {
	_v_att_sp->thrust_body[0] = -_last_thr_in_mc;
}

// 更新固定翼状态函数
void AbcVtol4R::update_fw_state() {
...
* 写入执行器输出数据
*/
void AbcVtol4R::fill_actuator_outputs() {
	// 初始化扭矩和推力设定值
...
// 检查前向转换是否完成
bool AbcVtol4R::isFrontTransitionCompletedBase() {
    ...
    */
	// 检查俯仰角是否达到阈值
	if (pitch <= ABC4R_PITCH_THRESHOLD_AUTO_TRANSITION_TO_FW) {
